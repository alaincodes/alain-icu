<template>
  <div class="grid place-content-center gap-7 max-w-4xl min-h-screen  w-full mx-auto p-6">
    <h1 class="text-3xl font-bold text-center">Alain Tran <br> Frontend Developer</h1>
    <h2 class="text-base font-bold text-center text-old-lace italic">Preparing new design for my website! <br> (Last update 22/05/2023)</h2>
  
    <ul class="grid grid-flow-col justify-center gap-6">
      <li>
        <a class="inline-block w-8 h-8 max-w-full" href="https://twitter.com/alaintran90">
          <img src="./assets/twitter-icon.svg" alt="">
          <span class="sr-only">my Twitter</span>
        </a>
      </li>
      <li>
        <a class="inline-block w-8 h-8 max-w-full" href="https://www.linkedin.com/in/alain-tran-6a95071ba/" target="_blank">
          <img src="./assets/linkedin-icon.svg" alt="">
          <span class="sr-only">my LinkedIn</span>
        </a>
      </li>
      <li>
        <a class="inline-block w-8 h-8 max-w-full" href="mailto:alaint77@gmail.com" target="_blank">
          <img src="./assets/at-icon.svg" alt="">
          <span class="sr-only">my E-mail</span>
        </a>
      </li>
    </ul>

    <div class="mx-auto">
      <h2 class="mb-7 text-2xl font-bold">Currently working for <a href="https://www.opsone.net/" target="_blank" class="inline-block p-2 text-philippine-orange bg-prussian-blue rotate-2 leading-7">Opsone</a> as <strong class="text-philippine-orange font-bold">Frontend Developer</strong> (since November 2019)</h2>
      <ul class="grid xs:grid-cols-2 gap-6">
        <li class="first-letter:mr-3 first-letter:text-5xl first-letter:font-bold first-letter:float-left first-letter:text-philippine-orange">
          Journeying into the world of frontend development specializing in 
          building awesome user interfaces for websites and apps. I bring 
          designs to life and make sure everything runs smoothly. I'm all about 
          making things look great, work well, and provide a user-friendly 
          experience.
        </li>
        <li class="first-letter:mr-3 first-letter:text-5xl first-letter:font-bold first-letter:float-left first-letter:text-philippine-orange">
          Simply put, I find joy in writing clean and organized code! I strive 
          for efficiency, following best practices and coding standards. This way, 
          the codebase is easy to understand, collaborate on, and ready for 
          future growth.
        </li>
      </ul>
    </div>
  
    <div class="grid xs:grid-cols-2 gap-6">
      <div>
        <p class="mb-4 text-2xl font-bold">Skills</p>
        <ul class="flex gap-4 flex-wrap">
          <li class="first-letter:font-bold text-philippine-orange">JavaScript</li>
          <li class="first-letter:font-bold">CSS</li>
          <li class="first-letter:font-bold">SCSS</li>
          <li class="first-letter:font-bold">TailwindCSS</li>
          <li class="first-letter:font-bold">Responsive UI</li>
        </ul>
      </div>

      <div>
        <p class="mb-4 text-2xl font-bold">Notable <strong class="text-philippine-orange font-bold">projects</strong> with my amazing team</p>
        <ul class="flex gap-4 flex-wrap">
          <li>
            <a href="https://www.lustucru-selection.fr" target="_blank" class="inline-block first-letter:font-bold">Lustucru Selection</a>
          </li>
          <li>
            <a href="https://www.panzanifoodservice.com" target="_blank" class="inline-block first-letter:font-bold">Panzani Food Service</a>
          </li>
          <li>
            <a href="https://www.opsone.net" target="_blank" class="inline-block first-letter:font-bold">Opsone</a>
          </li>
          <li>
            <a href="https://www.belairmonange.com" target="_blank" class="inline-block first-letter:font-bold">Bélair-Monange</a>
          </li>
          <li>
            <a href="https://www.delphiayachts.com" target="_blank" class="inline-block first-letter:font-bold">Delphiayachts</a>
          </li>
          <li>
            <a href="https://www.beneteau.com" target="_blank" class="inline-block first-letter:font-bold">Beneteau</a>
          </li>
          <li>
            <a href="https://www.timevallee.com" target="_blank" class="inline-block first-letter:font-bold">TimeVallée</a>
          </li>
          <li>
            <a href="https://www.enacr.com/" target="_blank" class="inline-block first-letter:font-bold">Enacr (Homepage)</a>
          </li>
        </ul>
      </div>
    </div>
  </div>
</template>

